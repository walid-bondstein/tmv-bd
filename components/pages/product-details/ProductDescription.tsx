...
export default function ProductDescription({
  ...
  const {
    productImage = "/images/storePlaceholder.png",
    productName,
    description,
    gallery,
    specifications,
  ...
  return (
    <div className="component-container mx-auto lg:my-24 my-7">
      <Tabs
        ...
        className="bg-[#F0F2F5] lg:p-12 sm:p-4 p-2 rounded-lg w-full"
      >
        <TabsList className="mx-0 w-full bg-transparent lg:h-[60px] h-[38px]">
          <TabsTrigger
            value="description"
            className="py-4 custom-tab lg:h-[60px] h-[38px] lg:text-2xl sm:text-base xs:text-base text-[12px] font-semibold cursor-pointer"
          >
            Description
          </TabsTrigger>
          <TabsTrigger
            value="gallery"
            className="py-4 custom-tab lg:h-[60px] h-[38px] lg:text-2xl sm:text-base xs:text-base text-[10px] font-semibold cursor-pointer "
          >
            Product Gallery
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="description" className="">
          <div className="lg:grid flex flex-col-reverse grid-cols-2 bg-[#F0F2F5]">
            <div className="bg-[#F0F2F5] p-4">
              <p className="text-lg lg:text-xl font-semibold md:text-2xl mb-4">
              ...
              <ul className="list-disc space-y-4 px-4 py-2">
                {description.map((desc, index) => (
                  <li
                    key={index}
                    className="text-base leading-7 text-[#687177] font-normal"
                  >
                    {desc}
                  </li>
                ))}
            ...
            <div className="bg-[#F0F2F5] p-4">
              ...
              <ul className="list-disc space-y-4 px-4 py-2">
                {specifications.map((desc, index) => (
                  <li
                    key={index}
                    className="text-base lg: leading-7 text-[#687177] font-normal"
                  >
                    {desc}
                  </li>
                ))}
